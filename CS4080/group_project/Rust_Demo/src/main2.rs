...
fn generate_init_population(count: u32, cities: &Vec<(f64, f64)>) -> Vec<CityItem> {
    ...
    for _ in 0..count {
        population.push(
            CityItem { path: path.clone(), city: city.clone()});
    }
...
impl Individual for CityItem {
    fn mutate(&mut self) {
        ...
        let index1: usize = random.gen_range(1, self.city.len());
        let mut index2: usize = random.gen_range(1, self.city.len());

        while index1 == index2 {
            index2 = random.gen_range(1, self.city.len());
        }
    ...
    fn calculate_fitness(&mut self) -> f64 {
        ...
        for index in &self.path {
            distance += calc_distance(&self.city, *prev_index, *index);
